...
3. raw bottom face + raw bottom audio human/visual lip-sync gate passed
4. every trim/dead-air/context cut uses the same cut list for top and bottom
5. no independent speed change, offset, retime, or frame shift on bottom face/audio
6. intermediate top/bottom video frame count and duration match, except documented container rounding
7. phase preview bottom face + bottom audio human/visual lip-sync gate passed before Phase 6/final render
8. caption cues are generated/remapped after final edited timeline is known
9. content cut strategy is lip-sync-safe: no bottom-face xfade while bottom circle is visible
```

## Metadata Is Not Lip-Sync Proof

```text
duration match, frame count match, fps match, and stream start_time match are necessary but not sufficient
metadata proves container alignment only
metadata does not prove that the mouth movement in the pixels matches the spoken consonants in the audio
if human review says the mouth is not aligned, the set fails even when ffprobe looks perfect
```

When multiple input sets exist:

```text
prefer the set whose bottom face + bottom audio pass human lip-sync review
block any set that fails human lip-sync review
do not continue a failed set to audio polish, captions, B-roll, or final render
```
