...
Added scripts/transcribe/{transcribe.py, word_segment.py, references/post-process-protocol.md}
Added scripts/clean-cut/{apply_edits.py, assembly.py, editorial_pass.py, format_transcript.py, install_vad.sh, preflight.sh, refine_edl.py, speech_to_edl.py, vad_detect.py, save_preference.sh, references/editorial-rules.md}
Added scripts/build-burst-captions.py — post-processed groups JSON → burst sub-composition
Added scripts/build-v88-composition.py — v87 template → v88 timing/source swap
Added compositions/captions-burst.html — particle-burst sub-composition
Added compositions/components/caption-particle-burst.html — installed via `npx hyperframes add caption-particle-burst`
Added V88_PLAYBOOK.md — portable step-by-step for new clips / new AI agents
Added SUBAGENT_PROMPTS.md — verbatim editorial + post-process prompts
Added .env.example + .env (gitignored) for ELEVENLABS_API_KEY
Replaced index.html with v88 composition (sub-composition mount on track 3 = removed timeline_track_too_dense)
Backed up old compositions to backups/index.v87.html and backups/index.v88-pre-burst.html
Created assets/v88-video-div/{top_v88_visual_master.mp4, bottom_v88_visual_master.mp4, speech_v88_polished.wav, transcript/{raw-elevenlabs.json, captions.json, words.json, caption-groups.json, meta.json}}
Created preview-v88/v88-video-div-burst-v2-final.mp4 (LOCKED REFERENCE)
```

Result:

```text
Final output: ../preview-v88/v88-video-div-burst-v2-final.mp4
Duration: 103.587s (video frames 3107, audio frames 4856, start_time 0.000)
Trim points: AI auto — start 24.70s, end 130.27s (was 24.0 user hint / ~127s v87)
False starts caught: 3 (opening abandoned hook 7.78-17.88, silent reset 17.88-24.90, outro [เสียงดนตรี])
Removed frames vs raw: 822 = 720 opening false-start trim + 89 dead-air (VAD-detected) + 13 outro tag
Caption system: 48 burst groups, 88 tokens (22 gold particle bursts + 66 white)
Caption timing: word boundaries from ElevenLabs (not interpolated)
Text fixes: 100,000 บาท / 20-30 ปี / หลายล้าน บาท / หุ้นสหรัฐฯ / dropped 4 "เนี่ย" fillers / AI+prompt+S&P 500 preserved Latin
BGM: Mixkit-1167 Close Up, 5%, frame-lock preserved through BGM mux
Loudness: -17.5 LUFS (target -16, within tolerance)
QA: npm run check pass 0 errors 0 warnings; check:caption-gold pass; 10 text elements pass WCAG AA; 0 layout issues
```

What v88 unlocks that v87 could not:

```text
1. AI decides start/end from spoken content — no user hint required
2. Editorial subagent catches false starts by content pattern, not just silence amplitude
3. Silero VAD respects Thai particles (ครับ/นะ/ของ) where ffmpeg silencedetect cut them
4. ElevenLabs Scribe v2 returns per-character timing for Thai (Whisper does not)
5. Post-process subagent produces caption text that matches what's actually spoken — no editor paraphrasing
6. Particle-burst kinetic captions with per-token gold highlighting on numbers + brands
7. timeline_track_too_dense warning eliminated by extracting captions to sub-composition
```

Cost / time on a clean 100s clip:

```text
ElevenLabs API: ~$0.024 (2× call — raw + edited timeline)
Editorial subagent: ~$0.30-0.60 (Claude Opus class)
Post-process subagent: ~$0.30-0.60
End-to-end: ~10-15 minutes including 2× HyperFrames render at 3 min each
```
