...
> **v88 supersedes parts of this file.** For new clips, use [`V88_PLAYBOOK.md`](V88_PLAYBOOK.md) Steps 1-15 which collapse Phases 4-10 into a portable 15-command pipeline. The phase content below stays as the v87-era reference; the v88 sections are marked inline.

## v88 → v87 phase mapping

| v87 Phase (below)             | v88 replacement                                                                |
| ----------------------------- | ------------------------------------------------------------------------------ |
| Phase 4 True Start/End        | Editorial subagent reads ElevenLabs transcript and produces edl-rough.json     |
| Phase 5 Parallel Trim         | Apply edl-rough → cleaned-rough.mp4; Silero VAD on post-rough → edl-jump.json  |
| Phase 6 Audio Polish          | Same chain (2-pass loudnorm + apad)                                            |
| Phase 7 Transcript            | ElevenLabs Scribe v2 (Whisper retired); re-transcribe polished on edited timeline |
| Phase 8 Context Index + Cut   | Skip (editorial subagent has already done meaning-based cuts in Phase 4)       |
| Phase 9 B-roll                | Same — Pexels/OpenRouter with reused stock                                     |
| Phase 10.1 Caption Build      | Post-process subagent reads polished transcript → caption-groups.json          |
| Phase 10.2 Composition        | Sub-composition mount (`compositions/captions-burst.html`) — removes timeline_track_too_dense |
| Phase 10.3-10.5 Render/Mux/QA | Same — visual-only render → speech mux → BGM 5% → frame-lock QA                |
