...
**For a new clip or porting to a different AI agent, start here:**

```text
V88_PLAYBOOK.md       Step-by-step (15 commands) for any clip with any AI agent
SUBAGENT_PROMPTS.md   Verbatim editorial + post-process subagent prompts
.env.example          Required API key template (ELEVENLABS_API_KEY)
npm run preflight     Verify ffmpeg/python/silero/keys before starting
```

**v88 changes (vs v87):**

```text
Phase 4 (True Start/End)  : User-hint trim → Editorial subagent reads transcript and finds false starts auto
Phase 5 (Dead Air)        : ffmpeg silencedetect → Silero VAD (preserves Thai particles like นะครับ/ของ)
Phase 7 (Transcript)      : Whisper large-v3 → ElevenLabs Scribe v2 (per-character Thai timing, brand-Latin)
Phase 10.1 (Caption Build): Editor-paraphrased text → Post-process subagent matches actual spoken words
Phase 10.2 (Composition)  : Inline captions → Sub-composition mount (removes timeline_track_too_dense warning)
Caption render style      : Static gold spans → Particle-burst (per-token reveal, 22 gold + 66 white)
Subagent prompts          : SUBAGENT_PROMPTS.md sections A and B (verbatim, portable)
Portability               : V88_PLAYBOOK.md walks any AI agent through Steps 1-15 on a new clip
```

**v88 locked output:** `../preview-v88/v88-video-div-burst-v2-final.mp4` — 103.587s, 3107 frames, 0 errors / 0 warnings

## Documents

```text
V88_PLAYBOOK.md           v88 portable step-by-step สำหรับ clip ใหม่ / AI agent อื่น
SUBAGENT_PROMPTS.md       verbatim editorial + post-process subagent prompts (v88)
