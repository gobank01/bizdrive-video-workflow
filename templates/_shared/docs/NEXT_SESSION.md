...
## Locked v88 Reference

```text
Final MP4:       ../preview-v88/v88-video-div-burst-v2-final.mp4
Duration:        103.587s (3107 video frames @ 30fps, 4856 audio frames AAC 48k)
Source raw:      ../video div/{top.mp4, bottom.mp4, bg.png} (130.517s untrimmed)
Trim by AI:      start 24.70s, end 130.27s (false start + silent reset auto-dropped)
Caption system:  particle-burst, 48 groups, 88 tokens (22 gold + 66 white)
Caption timing:  ElevenLabs word boundaries on edited timeline (not interpolated)
BGM:             Mixkit-1167 Close Up at 5%, frame-lock preserved
```

## What is the v88 baseline

```text
The pipeline is now portable: any AI agent (Claude, GPT, Gemini, Codex) with the prompts in
SUBAGENT_PROMPTS.md can reproduce equivalent output on any new clip following V88_PLAYBOOK.md.

Two AI-driven steps:
  Step 3 (V88_PLAYBOOK.md):  Editorial subagent → edl-rough.json
  Step 10 (V88_PLAYBOOK.md): Post-process subagent → caption-groups.json

Everything else is deterministic Python/ffmpeg/Node — no AI dependencies.
```

## How to start a new clip in this workflow

```text
1. Read V88_PLAYBOOK.md end-to-end (one-time familiarization)
2. Drop the new clip's top.mp4 + bottom.mp4 + bg.png in a known location
3. Pick a <JOB> slug (e.g., clip-2026-05-25-promo)
4. Run npm run preflight — fix any "no" before proceeding
5. Follow V88_PLAYBOOK.md Steps 1-15 in order
6. When you reach Step 3 and Step 10, spawn a subagent with the prompts in SUBAGENT_PROMPTS.md
7. Final output lives in ../preview-<JOB>/<JOB>-final.mp4
```

## How to port this to a different AI agent

```text
- Pipeline is stateless. SUBAGENT_PROMPTS.md sections A and B are full self-contained prompts.
- Drop them into Claude API / Codex CLI / Cursor / ChatGPT — all that's needed is filesystem read for
  raw-elevenlabs.json + editorial-rules.md, and filesystem write for the output JSON.
- Validate against the golden test in V88_PLAYBOOK.md section 10.
```

## v88 known imperfections (do not silently "fix" — document if you ramp them)

```text
1. Sparse keyframes in visual masters (GOP 8.37s, not 30 frames). Cosmetic for our render.
2. Loudness lands at -17.5 LUFS (target -16). Acceptable for social media.
3. BGM mix duration +21ms (audio metadata only, video frames identical).
4. 3-5 silences >0.5s in polished WAV (from Silero VAD pad_ms=200).
5. Caption timing on sub-word ElevenLabs entries: char-proportional interpolation, expect ~300ms drift on those tokens.
```

## ⚠️ Security note

```text
The ELEVENLABS_API_KEY currently in stacked-video/.env was exposed in the v88 session chat.
Rotate before sharing this repo or running on new infrastructure.
```
