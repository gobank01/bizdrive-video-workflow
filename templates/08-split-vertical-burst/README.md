# Template 08 — Split Vertical (Rectangle) with Karaoke Highlight Captions

**Created 2026-06-08 (v88).** Top/bottom split — top frame is the screen
(top.mp4), bottom frame is the **person as a full-width rectangle (no circle
crop)**, and the **BIZDRIVE Karaoke captions (caption-highlight word-sweep) sit
centered over the seam**. Layout cousin of Template 01/05; caption system is
identical to Template 04/05 (red box for normal words, gold box for
brand / number / tech tokens).
...
Layout:  bg.png full-frame → top frame (rounded rectangle, top half) + bottom frame (rectangle, face) → karaoke captions centered over the seam → B-roll inserts replace top frame contents
Captions: caption-highlight karaoke word-sweep, Thai-Latin mixed, gold box for numbers/brands
...
11. Build composition: set-duration.py <dur> → build-highlight-captions.py <groups> compositions/captions-highlight.html <dur> 910 → (optional) add-broll.py
...
compositions/
  captions-highlight.html      karaoke caption-highlight sub-composition (regenerated per job from caption-groups.json by build-highlight-captions.py)
  captions-burst.html          legacy particle-burst sub-composition (kept for reference; no longer mounted)
  components/
    caption-highlight.html       caption-highlight block from `npx hyperframes add` (reference — active caption system)
    caption-particle-burst.html  legacy particle-burst block (reference)
scripts/
  set-duration.py              set composition duration across index.html (run per job)
  build-highlight-captions.py  caption-groups.json → captions-highlight.html (karaoke word-sweep; index.html already mounts it)
  build-burst-captions.py      legacy particle-burst builder (kept for reference)
