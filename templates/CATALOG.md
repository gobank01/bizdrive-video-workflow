...
| _(no render yet)_ | **09 — Side-by-Side Two-Person (Vertical) with Particle-Burst Captions**<br>📐 9:16 · 1080×1920 · 30fps<br>💬 Captions: **BIZDRIVE particle-burst — each caption group pops word-by-word with a 10-particle burst; brand/number/tech tokens render gold (#FFD700). Same system + colors as Template 01.**<br><br>BIZDRIVE-style vertical 2-person format: two talking-head cams placed SIDE BY SIDE (left + right rectangles) in a 1080×1920 frame — an interview / conversation / 2-cam podcast layout — with centered particle-burst captions (same word-pop + gold #FFD700 system as Template 01) sitting below the two cams. Audio master is a single combined track (bottom.mp4 = the room/interview audio carrying both voices); the second cam (top.mp4) is muted video only. Layout is a new family (side-by-side); caption system = particle-burst (Template 01 colors).<br><br>📂 `templates/09-sidebyside-vertical-burst/` · scaffold: `bash tools/new-job.sh 09 <slug> --raw <raw>` |
