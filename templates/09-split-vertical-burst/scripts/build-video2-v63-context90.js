...
const VERSION = 63;
const COMPOSITION_DURATION = 89.32;
const XFADE = 0.12;
const FPS = 30;
const TOP_SOURCE = "assets/video2/top.mp4";
const BOTTOM_SOURCE = "assets/video2/bottom_audio_polished.mp4";
const TOP_OUTPUT = "assets/video2/top_context90_v63_softcut.mp4";
const BOTTOM_OUTPUT = "assets/video2/bottom_context90_v63_audio_polished_softcut.mp4";
...
const keepSegments = [
  {
    ...
    speech: "ตอนนี้ AI สามารถตัดต่อได้แล้ว ตัดต่อได้เนียนเหมือนคน",
...
const brollSources = [
  {
    id: "broll01",
    start: 5,
    output: "assets/broll/optimized/video2-v63/broll-01.mp4",
  ...
  },
  {
    id: "broll02",
    start: 14,
    output: "assets/broll/optimized/video2-v63/broll-02.mp4",
    source: "assets/broll/stock/pexels/test2-v35-full/slot-03/two-source-video-production-36265958-1920x1080.mp4",
    keyword: "editing_insert_zoom_caption",
    query: "video production control room",
    contextSegment: "features",
  },
  {
    id: "broll03",
    start: 23,
    output: "assets/broll/optimized/video2-v63/broll-03.mp4",
  ...
  },
  {
    id: "broll04",
    start: 32,
    output: "assets/broll/optimized/video2-v63/broll-04.mp4",
    source: "assets/broll/stock/pexels/test2-v35-full/slot-02/ai-automation-video-editing-32386590-1920x1080.mp4",
    keyword: "ai_processing_video",
    query: "advanced automation robotics",
    contextSegment: "technology_ready",
  },
  {
    id: "broll05",
    start: 41,
    output: "assets/broll/optimized/video2-v63/broll-05.mp4",
  ...
  },
  {
    id: "broll06",
    start: 50,
    output: "assets/broll/optimized/video2-v63/broll-06.mp4",
  ...
  },
  {
    id: "broll07",
    start: 68,
    output: "assets/broll/optimized/video2-v63/broll-07.mp4",
    source: "assets/broll/stock/pexels/test2-v35-full/slot-03/two-source-video-production-6883836-4096x2160.mp4",
    keyword: "creator_final_export",
    query: "video production monitor",
    contextSegment: "assistant_benefit",
  },
  {
    id: "broll08",
    start: 82,
    output: "assets/broll/optimized/video2-v63/broll-08.mp4",
    source: "assets/broll/stock/pexels/test2-v32-extra/slot-10/intro-outro-thumbnail-export-12691850-1920x1080.mp4",
...
function buildAudioFilter() {
  ...
  lines.push(`[${current}]loudnorm=I=-16:TP=-1.5:LRA=11[aout]`);
  return lines.join(";");
}
...
function renderBrollMedia() {
  ensureDir("assets/broll/optimized/video2-v63");
...
function buildBrollTags() {
  return brollSources
    .map((source, index) => {
      ...
      const mode = [2, 3, 6].includes(index) ? "bridge" : "soft";
...
function writeContextAndManifest() {
  ...
  const brollSlots = brollSources.map((slot, index) => {
    ...
    return {
      ...
      query: slot.query,
      usage: "reused_source_reencoded_for_video2_v63",
  ...
  const context = {
    version: VERSION,
    setId: "video2-v63-context90",
    goal: "Cut video2 from 115.946s to about 1:30 while preserving the complete AI video editing message and CTA.",
    strategy: "Use transcript/audio as primary context, drop repeated explanation, preserve hook/features/technology/benefit/CTA, soft cut all content cuts, and reuse QA-passed B-roll with comfortable spacing.",
  ...
  fs.writeFileSync("assets/context/video2-v63-context90.json", `${JSON.stringify(context, null, 2)}\n`);

  const manifest = {
    version: VERSION,
    setId: "video2-v63-context90",
    createdFor: "stacked-output-v63-video2-context90-full.mp4",
    summary: {
      ...
      minStartGapSeconds: 6,
      minRealTopFootageGapSeconds: 3,
      note: "Video2 v63 reuses local Pexels stock but re-encodes a clean job-specific B-roll set. Three earlier optimized candidates were rejected during final QA because they had visible graphic/text risk.",
    },
    qaRule: "Reject any B-roll with visible text, logo, watermark, other brand, distracting graphic, or poor relevance.",
    slots: brollSlots,
    rejectedCandidates: [
      {
        candidate: "assets/broll/optimized/test2-v35/broll-05.mp4",
        reason: "Rejected for visible digital graphic style; replaced with real automation footage.",
      },
      {
        candidate: "assets/broll/optimized/test2-v35/broll-09.mp4",
        reason: "Rejected for visible abstract symbol/graphic risk; replaced with real workshop footage.",
      },
      {
        candidate: "assets/broll/optimized/test2-v35/broll-04.mp4",
        reason: "Rejected for visible screen/text risk in the workflow handoff slot; replaced with cleaner creator desk footage.",
      },
    ],
  };
  fs.writeFileSync("assets/broll/optimized/video2-v63/manifest.json", `${JSON.stringify(manifest, null, 2)}\n`);
}
