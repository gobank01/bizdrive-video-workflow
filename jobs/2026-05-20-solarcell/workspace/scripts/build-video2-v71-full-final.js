...
const TOP_OUTPUT = "assets/video2/top_context90_v71_lipsync_safe.mp4";
const BOTTOM_OUTPUT = "assets/video2/bottom_context90_v71_lipsync_safe.mp4";
...
const brollSources = [
  {
    id: "broll01",
    start: 5,
    output: "assets/broll/optimized/video2-v71/broll-01.mp4",
  ...
  },
  {
    id: "broll02",
    start: 22,
    output: "assets/broll/optimized/video2-v71/broll-02.mp4",
  ...
  },
  {
    id: "broll03",
    start: 39,
    output: "assets/broll/optimized/video2-v71/broll-03.mp4",
  ...
  },
  {
    id: "broll04",
    start: 56,
    output: "assets/broll/optimized/video2-v71/broll-04.mp4",
  ...
  },
  {
    id: "broll05",
    start: 73,
    output: "assets/broll/optimized/video2-v71/broll-05.mp4",
...
function renderLipSyncSafeMedia() {
...
function renderBrollMedia() {
  ensureDir("assets/broll/optimized/video2-v71");
...
function writeContextAndManifest() {
  ...
  const keptSegments = keepSegments.map((segment, index) => {
    ...
    return {
      ...
      cutPlan: "lip-sync-safe hard concat; no visible bottom-face xfade; B-roll/top transitions provide smoothness",
      bottomCutMode: "hard_concat_no_xfade",
    };
  });
  ...
  const brollSlots = brollSources.map((slot, index) => {
    ...
    return {
      ...
      query: slot.query,
      usage: "reused_source_reencoded_for_video2_v71_lipsync_safe_final",
  ...
  const context = {
    version: VERSION,
    setId: "video2-v71-lipsync-safe-final",
    goal: "Re-edit the full video from raw sources with every current rule active, especially v69 lip-sync-safe cuts and v70 timestamped clip QA.",
    strategy: "Use bottom audio as master, drop the early false start before sustained speech, hard-concat bottom face/audio with no visible bottom xfade, keep top on the same timeline, use B-roll/top transitions for smoothness, and verify with cut contact sheets plus 1-second timestamp QA.",
    ...
    audioRepair: {
      ...
      syncAudioDelayMs: 0,
      reason: "v71 does not use metadata-only compensation before final QA; bottom face/audio are rebuilt together from the same hard-concat filtergraph.",
    },
    brollSpacing: {
  ...
  fs.writeFileSync("assets/context/video2-v71-lipsync-safe-final.json", `${JSON.stringify(context, null, 2)}\n`);

  const manifest = {
    version: VERSION,
    setId: "video2-v71-lipsync-safe-final",
    createdFor: "stacked-output-v71-video2-lipsync-safe-final.mp4",
    summary: {
      ...
      freshDownloadAttempted: true,
      freshDownloadStatus: process.env.PEXELS_API_KEY ? "available" : "blocked_missing_env_pexels_api_key",
      note: "v71 attempted fresh sourcing first, but the shell has no PEXELS_API_KEY/OPENROUTER_API_KEY, so local QA-passed stock was reused and re-encoded for the final run.",
    },
    qaRule: "Reject any B-roll with visible text, logo, watermark, other brand, distracting graphic, or poor relevance.",
    slots: brollSlots,
    rejectedCandidates: [],
  };
  fs.writeFileSync("assets/broll/optimized/video2-v71/manifest.json", `${JSON.stringify(manifest, null, 2)}\n`);
}

function main() {
  renderLipSyncSafeMedia();
  renderBrollMedia();
  buildIndex();
